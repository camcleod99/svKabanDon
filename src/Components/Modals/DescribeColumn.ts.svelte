<script lang="ts">
  ...
  import {describeColumn, readOneColumn, renameColumn} from "../../Controllers/columns";
  ...
  let oldColumnDescription = '';

  // Get the variables for the board
  onMount(async() => {
    id = get(activeId);
    console.log(id)
    const column = await readOneColumn(id);
    if (!column){
      columnDescription = '⚠️️ No Column Found in DB';
    } else {
      columnDescription = column.description;
      oldColumnDescription = column.description;
    }
  })
  ...
  function submitForm() {
    ...
    if (!columnDescriptionError) {
      describeColumn(id, columnDescription);
      closeModal();
    } else {
      window.alert(columnDescriptionError);
    }
  }
  ...
  function validateInput(input: string): string {
    if (!input) {
      ...
      return errorText[1];
    } else if (input === oldColumnDescription) {
      return errorText[2];
    } else {
      return '';
    }
      ...
      <div class="flex-grow">
        ...
        <form class="flex flex-col gap-5"
          ...
          <div class="flex flex-col gap-1">
            <label for="column_description" class="text-gray-800">Description <span class="text-red-500">{columnDescriptionError}</span></label>
            <input type="text" id="column_description" name="column_description" bind:value={columnDescription} class="p-2 border border-gray-400 rounded focus:outline-none focus:border-blue-500"/>
          </div>
        </form>
      </div>
    </div>
