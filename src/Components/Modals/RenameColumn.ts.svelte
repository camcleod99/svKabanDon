<script lang="ts">
  ...
  import { readOneColumn, renameColumn} from "../../Controllers/columns";
  ...
  onMount(async() => {
    ...
    if (!column){
      columnName = '⚠️️ No Column Found in DB';
    } else {
  ...
  function submitForm() {
    ...
    if (!columnNameError) {
      renameColumn(id, columnName);
      closeModal();
    } else {
      window.alert(columnNameError);
    }
  }
      ...
      <div class="flex-grow">
        ...
        <form class="flex flex-col gap-5"
          ...
          <div class="flex flex-col gap-1">
            <label for="task_name" class="text-gray-800">Board Name <span class="text-red-500">{columnNameError}</span></label>
            <input type="text" id="column_name" name="column_name" bind:value={columnName} class="p-2 border border-gray-400 rounded focus:outline-none focus:border-blue-500"/>
          </div>
        </form>
      </div>
    </div>
